 ...
 * first wins, so callers pass them in registry order. Articles outside the retention window are
 * dropped. The result is sorted newest first, ties by id, so the same data always serializes to the
 * same bytes. The arguments are not modified.
 */
export function mergeArticles(existing: Article[], incoming: Article[], now: Date): Article[] {
  ...
  return [...byId.values()].filter((article) => isWithinWindow(article.publishedAt, now)).sort(newestFirst);
}
